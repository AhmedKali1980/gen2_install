...
def main():
    ...
    with token_gen.generate() as token:
        for start in range(0, total, args.batch_size):
            ...
            for row in batch:
                ...
                assoc = associate_module(server_id, account_id, payload, token)
                if 'error' in assoc:
                ...
                job_res = run_install(server_id, account_id, os_type, token)
            ...
            while running:
                ...
                for row in running:
                    ...
                    status_obj = get_job_status(job_id, account_id, token)
                    ...
                    if status in FINAL_STATES:
                        dis = dissociate_module(server_id, account_id, token)
                        if 'error' in dis:
    ...
    print(f"\nEND: installation terminée. Résultats: {output_csv}")
